...
const CommonComponent = (data: any = {}) => {
  ...
  switch (info?.__typename) {
    case "ComponentComponentAboutUsComponent":
      return info?.title ||
        ...
        <About
          ...
          image={
            info?.image?.data?.attributes?.url !== undefined
              ? `${process.env.NEXT_PUBLIC_API_URL}${info.image?.data?.attributes?.url}`
              : ""
          }
    ...
    case "ComponentComponentCertificatesComponent":
      return info?.title || info?.desciption || info?.images ? (
        <div
          className={`titleCard`}
          style={
        ...
        >
          <div className="container">
            ...
            {info?.desciption && (
              ...
              >
                {useFormattedText(info.desciption)}
              </p>
            )}
            {info?.images?.length && (
              <Carousel
    ...
    case "ComponentComponentOurTeamComponent":
      return info?.title || info?.desciption || info?.cards?.length > 0 ? (
        <div
          className={`titleCard`}
          style={
            info?.background_color
          ...
          {info?.banners.map((doc: any, ind: number) =>
            ...
            doc?.mobile_image?.data?.attributes?.url ? (
              <Banner
                ...
                description_font_size={doc?.description_font_size}
                description_font_type={doc?.description_font_type}
                description_font_weight={doc?.description_font_weight}
                body={doc.body}
    ...
    case "ComponentComponentTestimonialsComponent":
      return info?.cards?.length > 0 || info?.title || info?.desciption ? (
        <div
          className={`titleCard`}
          style={
            info.background_color
                      ...
                      <div key={ind}>
                        <Card
                          ...
                          font_weight_desc_card={
                            doc?.testi_description_font_weight
                          }
                          para={
                            doc.desciption?.length > 240
                              ? `${doc.desciption?.slice(0, 240)}...`
    ...
    case "ComponentComponentContactUsComponenet":
      return info?.title || info?.desciption || info?.contact_details ? (
        <Contact
          description_color={info.desciption_color}
          title_color={info.title_color}
          title={info.title}
    ...
    case "ComponentComponentFeatureComponent":
      return info?.title || info?.description || info?.cards?.length > 0 ? (
        <div
          className={`titleCard`}
          style={
            info.background_color
        ...
        >
          <div className="container">
            ...
            {info?.description && (
              ...
              >
                {useFormattedText(info.description)}
              </p>
            )}
            {info?.cards?.length > 0 && (
              <Feature
    ...
    case "ComponentComponentPrivacyPolicy":
      return info?.Title || info?.Description || info?.DescriptionTitle ? (
        <div
          ...
          <div className="container">
            ...
            {info?.DescriptionTitle?.map((doc: any, i: number) => (
              <div key={i} className="policyDetail">
                {doc.Title && (
                  <p className="sub-lg">{useFormattedText(doc.Title)}</p>
                )}
                {doc.Description && (
                  <p className="para-md">{useFormattedText(doc.Description)}</p>
                )}
              </div>
            ))}
          </div>
